...
<head>
    ...
    <script>
        function check_input() {
            if(!document.memver.id.value){
                alert('아이디를 입력하세요!');
                document.member.id.focus();
                return;
            }
            if(!document.memver.pass.value){
                alert('비밀번호를 입력하세요!');
            ...
            if(!document.member.pass_confirm.value){
                alert('비밀번호확인을 입력하세요!');
                document.memver.pass_confirm.focus();
                return;
            }
            if(!document.member.name.value){
                alert('이름을 입력하세요!');
                document.memver.name.focus();
                return;
            }
            if(!document.member.email.value){
                alert('이메일 주소를 입력하세요!');
                document.memver.email.focus();
                return;
            }
            if(document.member.pass.value != document.member.pass_confirm.value){
                alert('비밀번호가 일치하지 않습니다. |n 다시 입력해 주세요!');
                document.member.pass.focus();
